...
//    P( PWR | N1,          SEND(KC_LSFT); SEND(KC_GRV));     // ~

    P( PWR | ST3,          SEND(KC_QUOT));

    P( PWR | ST4,          SEND(KC_LSFT); SEND(KC_QUOT));    // "
//    P( PWR | N7,          SEND(KC_LSFT); SEND(KC_GRV));     // ~
...
*/
// AK: Added placeholders for the keys not present in Georgi's hardware
// Main layer, everything goes through here
    [STENO_LAYER] = LAYOUT_georgi(
